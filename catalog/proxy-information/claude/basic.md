...
这是 2026-09-20 Catalog 1.7.0 的目标，不是本机安装状态：Proxy **0.3.1**，Runtime **2.1.159**。

目标位置样例：

```text
<dataDir>/runtimes/claude/2.1.159/<artifact-sha256>/bin/claude
```
...
| 有源码 Proxy，无 Runtime | 安装位置：Host 返回的计划位置 | 未安装 | 0.3.1，标明开发来源 | 不把源码存在当成认证组合已激活 |
...
- Claude Code 的 Proxy 版本、Runtime 版本与 Gian App 版本不是同一个版本号。
