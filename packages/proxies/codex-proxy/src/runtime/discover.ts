...
const COMMAND = 'codex';
const SETUP_URL = 'https://chatgpt.com/codex/install.sh';
...
export async function discoverCodexRuntimes(): Promise<{
  ...
  for (const directory of (process.env.PATH ?? '').split(delimiter)) {
    ...
    seen.add(path);
    candidates.push({ path, source: 'path', label: 'Codex CLI' });
  }
  for (const official of [
    ...
    candidates.push({ ...official, label: 'Codex CLI' });
  }
  return {
    candidates,
    setupActions: [
      { id: 'docs', kind: 'open_url', label: 'Install Codex', url: SETUP_URL },
      { id: 'pick-binary', kind: 'select_file', label: 'Choose Codex binary' },
    ],
  };
}

async function runVersion(path: string): Promise<string> {
  const result = await runBoundedCommand(path, ['--version'], { timeoutMs: 8_000 });
  const version = firstVersion(`${result.stdout}\n${result.stderr}`);
  if (!version) throw new Error('`codex --version` did not report a semantic version');
  return version;
}

export async function probeCodexRuntime(path: string): Promise<{
  runtimeId: string;
  displayName: string;
  path: string;
  version: string;
  ...
  if (!isAbsolute(path)) throw new Error('Codex runtime path must be absolute.');
  await access(path, constants.X_OK);
  const version = await runVersion(path);
  const configured = process.env.CODEX_HOME;
  const configHome = configured && isAbsolute(configured) ? configured : join(homeDir(), '.codex');
  return {
    runtimeId: 'codex',
