/** Per-process dispatch queue for the cc-proxy CLI spawn loop.
 *
 *  Two traffic classes share one process:
 ...
 *    only after the previous one has settled. That is what keeps per-request
 *    notification capture slots (the adapter's deferred queue) from being
 *    clobbered by a concurrent session handler, and what preserves
 *    shutdown/sidechat ordering.
